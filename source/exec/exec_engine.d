	...
	void execute_instr(Instruction instr) {
...

				{
					...
					if (stack_frame !is null) {
						...
						while (!stack.is_empty()) {
							logger.Info(
									"caching stack, stack_ptr is " ~ to!string(
									stack.stack_ptr));
		...
		case OP.RET: {
				...
				if (prev_frame !is null && curr_stack_frame() !is null) {
					...
					logger.Verbose("Jumping to addr" ~ to!string(prev_frame
							.return_addr));
		...
		case OP.LDI: {
				...
				logger.Verbose("Loaded " ~ to!string(val) ~ " from addr " ~ to!string(
						addr));
				break;
			}
		...
		case OP.ALLOCI: {
				...
				logger.Verbose("Alloc'd local " ~ to!string(val) ~ " at addr " ~ to!string(
						addr));
				break;
			}
		...
		case OP.STRI: {
				...
				logger.Verbose("Stored " ~ to!string(val) ~ " at addr " ~ to!string(
						addr));
				break;
			}
