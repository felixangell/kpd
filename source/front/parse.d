...
class Parser : Compilation_Phase
{
    ...
    Token expect(Token_Type type)
    {
        if (!peek().cmp(type))
        {
            ...
            err_logger.Error(peek(), "Expected '" ~ to!string(
            ...
            if (peek().cmp("="))
            {
                ...
                if (value is null)
                {
                    err_logger.Error(peek(),
                            "expected value after assignment operator in structure field: ");
                    recovery_skip("}");
                    break;
                }
            }
    ...
    ast.Paren_Expression_Node parse_paren_expr()
    {
        ...
        {
            err_logger.Error(peek(),
                    "Expected an expression inside of parenthesis expression, found:");
            recovery_skip(")");
        }
        expect(")");
