...
class Lexer : Compilation_Phase {
	...
	Token recognize_num() {
		...
		case 'X':
			...
			return new Token(prefix ~ consume_while(is_hexadecimal), Token_Type
					.Integer_Literal);
		case 'o':
		case 'O':
			string prefix = sign ~ to!string(consume()) ~ to!string(consume());
			return new Token(prefix ~ consume_while(is_octal), Token_Type
					.Integer_Literal);
		case 'b':
		case 'B':
			string prefix = sign ~ to!string(consume()) ~ to!string(consume());
			return new Token(prefix ~ consume_while(is_binary), Token_Type
					.Integer_Literal);
		case 'd':
		case 'D':
			string prefix = sign ~ to!string(consume()) ~ to!string(consume());
			return new Token(prefix ~ consume_while(is_decimal), Token_Type
					.Integer_Literal);
		default:
			if (isAlpha(peek(1))) {
				writeln("TODO: o fuck\n");
		...
		Token tok = new Token(consume_while(is_decimal), Token_Type
				.Integer_Literal);
	...
	Token[] tokenize() {
		...
		while (has_next()) {
			...
			if (recognized_token !is null) {
				...
				recognized_token.position = new Span(start_loc, end, tok_stream
						.length - 1);
